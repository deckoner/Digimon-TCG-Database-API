...
# * Cards list
def get_all_cards(page=1, per_page=10, include_alternative=True):
    """
    Fetches all cards with full descriptive data by replacing foreign key IDs with their names,
    with optional filtering for alternative artwork.
    Supports pagination.
    """
...
def get_all_cards_full_info(page=1, per_page=10, include_alternative=True):
    """
    Fetches all cards with full descriptive data by replacing foreign key IDs with their names,
    with optional filtering for alternative artwork.
    Supports pagination.
...
def search_cards_by_name(name_part):
    ...
    Args:
        name_part (str): Substring to search for in card names.
...
def search_cards_with_alternatives_by_name(name_part):
    """
    Searches all main cards (non-alternative) whose names contain the given substring,
    and includes their alternative versions, all in a single query.

    Args:
        name_part (str): Substring to search for in card names.

    Returns:
        list[dict]: List of main cards, each with an 'alternatives' key containing a list of its alternative versions.
    """
    connection = _create_connection()
    if not connection:
        return []
    ...
    try:
        ...
        query = """
            ...
            ORDER BY main.card_number, alt.card_number
        """
        cursor.execute(query, (search_term,))
        ...
        for row in rows:
            ...
            if key not in grouped:
                # Extraer solo los campos de la carta principal
                grouped[key] = {
                    "id": row["main_id"],
            ...
            # Si hay alternativa en la fila, añadirla
            if row["alt_id"]:
...
def add_card_to_deck(deck_id, card_number, quantity):
    """
    Adds a card to a deck. If it exists, increases its quantity.
