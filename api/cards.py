...
def list_cards(
    ...
    cards = get_all_cards(page, per_page, include_alternative)
    if cards is None:
        # This indicates that there was a problem with the query or server
        raise HTTPException(status_code=500, detail="Internal server error retrieving cards")
    return cards
...
def list_cards_full_info(
    ...
    cards = get_all_cards_full_info(page, per_page, include_alternative)
    if cards is None:
        # This indicates that there was a problem with the query or server
        raise HTTPException(status_code=500, detail="Internal server error retrieving cards")
    return cards
...
def get_card(card_number: str):
    try:
        ...
        # This indicates that there was a problem with the query or server
        raise HTTPException(status_code=500, detail="Internal server error retrieving card")
    if not card:
...
def search_cards(name_part: str = Query(..., min_length=2, description="Partial card name to search")):
    try:
    ...
    if not cards:
        # This indicates that the chart is not in the database.
        raise HTTPException(status_code=204, detail="No cards matching the search found")
    return cards

@router.get("/search-with-alternatives/", summary="Search cards with alternatives")
def search_cards_with_alternatives(name_part: str = Query(..., min_length=2, description="Partial card name to search")):
    try:
